  ...
  if (!seedPromise) {
    seedPromise = (async () => {
      ...
      // (nieopłacone/po terminie), pozycji ani stanu wpłat.
      {
        ...
        // Katalog komponentów (Moduł 47) zasiewa się sam przez `seedStarterCatalog()`
        // w `createInvoicesSchema()` (ta sama biblioteka co na produkcji —
        // `lib/catalogStarter.ts`), więc tu nic nie dokładamy.
