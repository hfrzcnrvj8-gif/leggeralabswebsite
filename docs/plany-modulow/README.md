...
| 35 | ⏳ **Projekty: zakładki zamiast ściany + layout tablicy** — `ProjectDetailPanel.tsx` ma **1779 linii wobec 791** w kliencie (zweryfikowane). Wzorzec zakładek już istnieje (Klient, Moduł 23 — stan `tab` w `*DetailPanel`, więc działa i w modalu, i na podstronie). Propozycja: Podgląd / Onboarding / Czas pracy / Logi zmian — **podział do rozstrzygnięcia z właścicielem**. **Część B POTWIERDZONA zrzutami (2026-07-17): „góra zagospodarowana, dół wolny, bezużyteczny" — OBA widoki Projektów I podgląd Poczty.** Diagnoza gotowa: panel nie przekazuje wysokości ekranu w dół — `AppShell.tsx:450` (kolumna treści) jest rozciągnięta, ale `:459` (`mx-auto px-4 py-5`) ma już wysokość treści i tam łańcuch się urywa; dalej root dashboardu i kontener kolumn tak samo. Naprawa: `flex flex-col` + `flex-1 min-h-0` od AppShell do kontenera widoku (`min-h-0` kluczowe). **Główna decyzja: tylko Projekty+Poczta (zgłoszone) czy wszystkie moduły naraz** — mają ten sam łańcuch, więc wyrywkowo zostawi panel w połowie drogi | średni/duży | [35-projekty-zakladki-i-tablica.md](35-projekty-zakladki-i-tablica.md) |
