...
| 33 | ⏳ **NASTĘPNY — Ikony zamiast emoji w panelu** (emoji zostają w mailach; warunek „po 30 i 31" spełniony 2026-07-17) — dokończenie migracji z 2026-07-11, nie nowy kierunek: emoji pełnią dziś rolę **ikon systemowych** (`icon="🏢"`, foldery Poczty), a renderują się inaczej na każdym systemie, nie dziedziczą koloru i nie trzymają siatki. ~128 linii, ale `Menu.tsx` już przyjmuje `ReactNode` → **bez zmian typów**. **Wyjątek na stałe: podpis mailowy / mail dzienny zostają na emoji** (w HTML-u maila nie ma komponentów React). **Warunek „po 30 i 31" spełniony** (oba ✅ 2026-07-17). **Brief zweryfikowany gretem 2026-07-17** (sekcja „PRZECZYTAJ NAJPIERW"): liczby były zawyżone (realnie **113 linii** i **24** wywołania `icon="…"`, nie ~128/33), a **inwentaryzacja pomijała `lib/`** — ~50 linii chrome'u panelu (dzwonek, oś czasu klienta, kanały kontaktu; wszystkie zweryfikowane jako panel-only, nie kolidują z wyjątkiem mailowym). **Sprostowanie B:** „bez zmian typów" dotyczy tylko `Menu.tsx` — mapy w `lib/` to `Record<…,string>`, a `lib/` nie ma ani jednego `.tsx`, więc JSX tam nie wejdzie; doszło **pytanie 4** (gdzie mają mieszkać mapy ikon — propozycja: `shared.tsx`). `tsc` tego modułu NIE zweryfikuje — wymaga przeglądu wizualnego w obu motywach. Gotowy prompt: [PROMPT-33.md](PROMPT-33.md) | mały/średni, wizualny | [33-ikony-zamiast-emoji.md](33-ikony-zamiast-emoji.md) |
