...
| 35 | 🟡 **Część B ZROBIONA (2026-07-17), część A czeka** — **B: panel wypełnia wysokość okna, wszystkie moduły** (decyzja właściciela: „od razu wszędzie, wygląd ma być spójny"). Przyczyna była jedna: panel nie przekazywał wysokości w dół, więc `overflow-auto` nie miał się do czego odnieść i „wewnętrzne przewijanie" było fikcją. Naprawa: `md:h-screen md:overflow-hidden` + łańcuch `flex-1`/`min-h-0` od AppShell w dół; scroll panelu w kontenerze treści, żeby nieprzerobione moduły dalej działały. Usunięte magiczne liczby (`calc(100vh-260px)` w Poczcie, `calc(100vh-140px)` i `max-h-[70vh]` w Kalendarzu, `max-h-[70vh]` w tabelach) — **nie dokładaj nowych, to znak urwanego łańcucha**. Pułapka: flex-column rozciąga dzieci (`self-start`). Test: przy oknie 420px Pulpit przewija 1175px treści w środku — nic nie ucięte. **A (zakładki w profilu projektu — 1779 linii wobec 791 u klienta) NADAL OTWARTE.** Wyniki: `HUB_SETUP.md` → „Moduł 35, część B" | A: średni/duży | [35-projekty-zakladki-i-tablica.md](35-projekty-zakladki-i-tablica.md) |
