...
| 27 | ⏳ Gęstość ekranu (puste przestrzenie) + liquid glass w przełącznikach — kontynuacja Modułu 21. Brief odświeżony 2026-07-17 (zmierzone, nie na oko): realna szerokość treści to **1576 px, nie 1800** — **Faktury/Oferty** mają obok KPI **904 px martwego pasa** i to one są najostrzejsze, a **nie Koszty** (tam rząd jest pełny, pustka siedzi wewnątrz karty trendu). Część B sprowadza się do rozciągnięcia gotowego wzorca `ViewTabs` (`layoutId` + spring) na `FilterPills` (dziś zero animacji, aktywna pigułka przeskakuje) i segmenty Osi czasu. **Pułapka:** Notatnik renderuje dwa zestawy `FilterPills` naraz, więc stały `layoutId` powtórzy błąd z Modułu 23. Zacznij od pytań (czym wypełnić odzyskane miejsce, jak daleko ze szkłem) | mały/średni | [27-gestosc-i-liquid-glass.md](27-gestosc-i-liquid-glass.md) |
