...
| 30 | ⏳ **Powiązanie z Klientem** — korzeń łączący Kroki 2/6/10 drogi klienta: oferta/faktura założona „od zera" nie ma klienta, nic o tym nie mówi, a `INSERT` faktury **nie ma kolumny `client_id` w ogóle** → brak retencji, pusta karta klienta, pusta oś czasu. Domyka odłożoną pozycję Modułu 22. **Zacznij od pytań** (co `client_id` na fakturze znaczy: migawka czy powiązanie?) | średni, z audytu 29 | [30-powiazanie-z-klientem.md](30-powiazanie-z-klientem.md) |
...
| 32 | ⏳ **Teksty prowadzące sprzed Modułów 11–17** — mapa procesu (12 kroków bez Umowy/NDA/Onboardingu/Wsparcia) na każdej karcie leada; **podpowiedź każąca ustawić przypomnienie, którego panel nigdy nie pokaże** (`isOverdue()` odrzuca zamknięte leady, zanim spojrzy na `next_followup`); mapa drogi obiecuje 4 rzeczy, których nie ma. Panel-mentor mylący się co do własnych możliwości | mały/średni, z audytu 29 | [32-teksty-prowadzace.md](32-teksty-prowadzace.md) |
