...
| 31 | ⏳ **NASTĘPNY — Umowy: pułapka bramki + widoczność** (ostatni z trzech briefów audytu 29; 32 ✅ i 30 ✅ zamknięte) — projekt założony ręcznie **nigdy nie przejdzie na „W trakcie"** (umowy nie da się przypiąć do projektu z UI, choć serwer to potrafi). Umowy nieobecne na Pulpicie, w dziennym mailu, wyszukiwarce, karcie klienta i statystykach. Cisza po e-podpisie oferty. **Brief zweryfikowany gretem 2026-07-17** (sekcja „PRZECZYTAJ NAJPIERW" — wszystkie znaleziska potwierdzone co do linii, ale **rekomendowana „naprawa jednej linii" to pułapka**: `linkValueFor` jest wyłączne, więc dodanie `"project"` do `kinds` wyzeruje `client_id` umowy i cicho rozwali Część B). **Pytanie 1 rozstrzygnięte 2026-07-17**: bramka zostaje twarda, ale obejmuje **tylko projekty z `client_id`** — `POST /api/projects` nigdy nie ustawia klienta, więc dziś nawet „przebudowa własnej strony" nie ruszy z „W trakcie", a umowy nie ma z kim podpisać. **Zostają pytania 2–5** (w tym kolizja z decyzją Modułu 24 o dzwonku jako kronice). Gotowy prompt: [PROMPT-31.md](PROMPT-31.md) | średni, z audytu 29 | [31-umowy-widoczne.md](31-umowy-widoczne.md) |
