...
| 33 | ⏳ **NASTĘPNY — Ikony zamiast emoji w panelu** (emoji zostają w mailach; warunek „po 30 i 31" spełniony 2026-07-17) — dokończenie migracji z 2026-07-11, nie nowy kierunek: emoji pełnią dziś rolę **ikon systemowych** (`icon="🏢"`, foldery Poczty), a renderują się inaczej na każdym systemie, nie dziedziczą koloru i nie trzymają siatki. ~128 linii, ale `Menu.tsx` już przyjmuje `ReactNode` → **bez zmian typów**. **Wyjątek na stałe: podpis mailowy / mail dzienny zostają na emoji** (w HTML-u maila nie ma komponentów React). **Po 30 i 31** (decyzja właściciela 2026-07-17) | mały/średni, wizualny | [33-ikony-zamiast-emoji.md](33-ikony-zamiast-emoji.md) |
