...
| 37 | 🚧 **Standard branżowy (Linear/Notion) + eksport do Excela** — część A **wykonana 2026-07-22** (rozjazdy apka↔panel: jeden słownik koloru kalendarza w palecie marki, „dziś" bez czerwieni, jedna recepta rysowania wydarzenia na czterech poziomach, parytet Przypomnień). Część B (eksporty) i wspólne klocki apki **przeniesione do Modułu 38** wraz z odpowiedziami właściciela na cztery pytania o Excel. Zostaje jako zapis audytu i decyzji | duży | [37-standard-branzowy-i-excel.md](37-standard-branzowy-i-excel.md) |
| 38 | 🚧 **Eksporty CSV + reszta spójności** — **Część A ✅ 2026-07-22**: wiersz podsumowania (sprzedaż per waluta), eksport czasu pracy (linia = sesja, chodzący stoper wypada), eksport klientów i projektów (całe rejestry, bez zakresu dat — `start`/`termin` są opcjonalne i filtr gubiłby wiersze). Otwarta **Część B**: wspólne klocki apki (odznaka, stała promienia, tokeny czcionek — `.font(.system(size:))` urosło do 26). Zawiera listę pięciu „odstępstw", które okazały się świadomymi decyzjami — nie zgłaszaj ich drugi raz | średni | [38-eksporty-i-reszta-spojnosci.md](38-eksporty-i-reszta-spojnosci.md) |
