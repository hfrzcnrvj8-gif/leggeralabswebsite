...
| 40 | ⏭️ **NASTĘPNY. Unieważnianie publicznych linków + białe listy pól** — domknięcie Audytu 1 (2026-07-22). Pięć rodzajów tokenów jest dziś **wieczne i nieodwoływalne**; mail przesłany dalej = trwały dostęp. Decyzja właściciela: **bez automatycznego wygasania**, za to przycisk „Unieważnij link" + `revoked_at` — w **sześciu** zapytaniach, bo trzy z tych tras ZAPISUJĄ (podpis umowy starym linkiem!). Część B: czarne listy pól w `*/public/[token]` na białe — to przez nie wyciekły IP podpisującego i drugi token. Prompt: [PROMPT-40.md](PROMPT-40.md) | mały | [40-uniewaznianie-linkow.md](40-uniewaznianie-linkow.md) |
| 41 | 🔑 **Drugi składnik logowania (TOTP)** — właściciel poprosił o 2FA 2026-07-22, **znosząc zapis „jedno hasło" z `CLAUDE.md`**. Hamulec z Audytu 1 zamyka zgadywanie, nie zamyka wycieku hasła. Kody zapasowe obowiązkowe (nie ma administratora, który by odblokował konto) + wyłącznik awaryjny przez zmienną w Vercelu | średni | [41-drugi-skladnik-totp.md](41-drugi-skladnik-totp.md) |
