...
| 34b | ⏳ **NASTĘPNY — pigułki na wszystkich paskach** (dokończenie 34) — Moduł 34 podpiął rozsuwane pigułki **tylko pod pasek Leadów**, więc panel jest w połowie drogi: ikona w Leadach daje ładną pigułkę, ta sama w Fakturach — stary systemowy prostokąt. Dokładnie ten stan, przed którym ostrzega `CLAUDE.md`. Runda **mechaniczna**, zero decyzji produktowych. **Kluczowe: NIE zamieniać wszystkich 154 natywnych `title=`** — to cztery różne rzeczy: (1) ikona bez podpisu w pasku → pigułka; (2) przycisk z widocznym podpisem („Filtry") → usunąć `title`, to duplikat; (3) objaśnienie-zdanie na karcie KPI → dymek; (4) akcja w wierszu tabeli → dymek (pigułka zasłoniłaby dane). **`Truncate` zostaje na natywnym `title`** — tam niesie pełną uciętą treść. Zasada: **pigułka podpisuje ikonę, dymek tłumaczy stan**. Inwentarz: Faktury 13, Oferty 9, Koszty 6, Umowy 4, Projekty 3, Klienci 2, Notatnik 2. Gotowy prompt: [PROMPT-34b.md](PROMPT-34b.md) | mały/średni, mechaniczny | [34b-pigulki-wszedzie.md](34b-pigulki-wszedzie.md) |
| 35 | 🟡 **Część B ZROBIONA (2026-07-17), część A czeka** — **B: panel wypełnia wysokość okna, wszystkie moduły** (decyzja właściciela: „od razu wszędzie, wygląd ma być spójny"). Przyczyna była jedna: panel nie przekazywał wysokości w dół, więc `overflow-auto` nie miał się do czego odnieść i „wewnętrzne przewijanie" było fikcją. Naprawa: `md:h-screen md:overflow-hidden` + łańcuch `flex-1`/`min-h-0` od AppShell w dół; scroll panelu w kontenerze treści, żeby nieprzerobione moduły dalej działały. Usunięte magiczne liczby (`calc(100vh-260px)` w Poczcie, `calc(100vh-140px)` i `max-h-[70vh]` w Kalendarzu, `max-h-[70vh]` w tabelach) — **nie dokładaj nowych, to znak urwanego łańcucha**. Pułapka: flex-column rozciąga dzieci (`self-start`). Test: przy oknie 420px Pulpit przewija 1175px treści w środku — nic nie ucięte. **A (zakładki w profilu projektu) NADAL OTWARTE — NASTĘPNY MODUŁ.** Zmierzone na żywo: `ProjectDetailPanel.tsx` **1779 linii** wobec **791** u klienta, karta w modalu **1566 px** wobec **1017 px**. **To DWA braki, nie jeden:** poza zakładkami profil projektu **nie ma `max-h`**, które klient ma (`max-h-[85vh]`) — stąd karta wystaje poza okno. **Projekty NIE MAJĄ audytu zmian** (`field_changes` nie istnieje), więc zakładka „Logi zmian" to osobny zakres, nie „skopiuj od klienta". Wzorzec: Moduł 23 (stan `tab` w `*DetailPanel`, własny `layoutId`). Cztery pytania do właściciela. Wyniki B: `HUB_SETUP.md` → „Moduł 35, część B". Gotowy prompt: [PROMPT-35.md](PROMPT-35.md) | A: średni/duży | [35-projekty-zakladki-i-tablica.md](35-projekty-zakladki-i-tablica.md) |
| 36 | ⏳ **Animacje i lekkość — druga rata rundy z 2026-07-16** — nie nowy problem: tamta runda była świadomie pierwszym krokiem, z zapisem „gdy właściciel wróci z podobną uwagą, kontynuuj tę rundę". Wrócił 2026-07-17. **Zweryfikowany rozjazd: jedna krzywa `cubic-bezier(0.16,1,0.3,1)` ma 3 wystąpienia, a obok 7× `easeOut` i 11× `linear`**; `stiffness: 420` w 12 miejscach, ale są 500/400/120. `prefers-reduced-motion` ✅ obsłużone — nie ruszaj. **Rozstrzygnięte: „liquid glass" NIE dosłownie** — chodzi o lekkość, `.glass` zostaje tylko na chrome. **⚠️ Ten moduł jest o animacjach, a w podglądzie animacje potrafią nie działać wcale** (rAF = 0) — zawsze zaczynaj od `tabs_create` | średni, w 100% wizualny | [36-animacje-i-lekkosc.md](36-animacje-i-lekkosc.md) |
