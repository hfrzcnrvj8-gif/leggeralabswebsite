...
| 40 | 🔒 **Unieważnianie publicznych linków** — z Audytu 1 (2026-07-22). Pięć rodzajów tokenów (oferta, umowa, faktura, wezwanie, opinia) jest dziś **wieczne i nieodwoływalne**; mail przesłany dalej = trwały dostęp. Decyzja właściciela: **bez automatycznego wygasania**, za to przycisk „Unieważnij link" + `revoked_at`. Przy okazji zamienić czarne listy pól w trasach `*/public/[token]` na białe — to przez nie wyciekł `client_id` | mały | [40-uniewaznianie-linkow.md](40-uniewaznianie-linkow.md) |
