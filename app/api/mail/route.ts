...
import { MAIL_STATUSES, type MailMessageWithLinks } from "@/lib/mail";
...
export async function GET(req: NextRequest) {
  ...
  const rows = (await sql`
    SELECT m.id, m.uid, m.kierunek, m.client_id, m.lead_id, m.invoice_id,
           ...
           '' AS body_html,
           m.message_id, m.in_reply_to, m.refs, m.status, m.received_at, m.handled_at,
  ...
  const [counts] = (await sql`
    ...
    FROM mail_messages;
  `) as unknown as { nowe: number; nieprzypisane: number }[];
